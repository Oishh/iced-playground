...
use iced::{border, keyboard, mouse, Length, Point, Rectangle, Renderer, Theme};
use iced::widget::pane_grid::{self, PaneGrid};
use iced::widget::{
    button, canvas, center, column, container, horizontal_space, responsive, row, scrollable, text
};
...
impl Example {
    ...
    fn update(&mut self, message: Message) {
        match message {
            ...
            Message::BoardManagement => {
                self.board_management = !self.board_management;
            }
        }
    }
    ...
    fn view(&self) -> Element<Message> {
        ...
        let pane_grid = PaneGrid::new(&self.panes, |id, pane, is_maximized| {
            ...
            pane_grid::Content::new(responsive(move |size| {
                view_content(id, total_panes, pane.is_pinned, size, self.board_management)
            }))
            .title_bar(title_bar)
            .style(if is_focused {
                style::pane_focused
        ...
        let element: Element<Message> = container(pane_grid).padding(10).into();
...
fn view_content<'a>(
    ...
    let board_management = column![]
        .push_maybe(if board_management {
           Some(board_container)
        } else {
            None
        })
        .max_width(160);
    ...
    let content_2 =
        column![board_management]
            .spacing(10)
            .align_x(Right);

    let content = row![content_1, content_2].spacing(5);
...
fn view_controls<'a>(
    ...
    let board_management = button(text("Board Management").size(14))
    ...
    let buttons = row![row![board_management, close].spacing(5)];
